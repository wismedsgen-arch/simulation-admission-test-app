...
1. **Phase H3** — Per-cycle export. Add a cycle picker to `/admin/export` and a per-cycle download button (the route handler already accepts `?scope=cycle&cycleId=...`; `passwordHash` redaction is already wired in H1's builder).
2. **Phase H4 (future)** — Restore script `scripts/restore-export.ts`, mirroring the QA-reset script's safety harness (`ALLOW_RESTORE`, matching `--confirm` phrase, `--execute`, `--i-understand-production`, schema-fingerprint guard, sha256 verification, table-by-table inserts in dependency order, blob re-upload under original `storageKey`).
